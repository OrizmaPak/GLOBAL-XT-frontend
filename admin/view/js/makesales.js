let makesalesid
let themakesalesitems
let rptotalorder
let viewsaledaysource
// let initialinventoryload
async function makesalesActive() {
    document.getElementById('transactiondate').value = new Date().toISOString().split('T')[0];
    themakesalesitems = '';
    thedepartment = '';
    thebranch = '';
    initialinventoryload = ''
    // const form = document.querySelector('#makesalesform')
    // const form2 = document.querySelector('#updateinventories')
    if(document.querySelector('#salessubmit')) document.querySelector('#salessubmit').addEventListener('click', e=>makesalesFormEditHandler())
    if(document.querySelector('#viewsalesdaySubmit')) document.querySelector('#viewsalesdaySubmit').addEventListener('click', e=>fetchviewsalesday())
    if(document.querySelector('#viewsalesmonthSubmit')) document.querySelector('#viewsalesmonthSubmit').addEventListener('click', e=>fetchviewsalesmonth())
    if(document.querySelector('#viewsalesyearSubmit')) document.querySelector('#viewsalesyearSubmit').addEventListener('click', e=>fetchviewsalesyear())
    ...
    //     }
    // });
    if(document.getElementById('departmentfrom')) document.getElementById('departmentfrom').addEventListener('change', e=>makesaleschecksourcedetails())
 

}

function makesaleschecksourcedetails(){
    themakesalesitems = ''
    ...
    if(document.getElementById('branchfrom').value && document.getElementById('departmentfrom').value){
        document.getElementById('tabledata').innerHTML = `<tr><td colspan="100%" class="text-center opacity-70">Loading sales items...</td></tr>`;
        document.getElementById('entrybody').innerHTML = `<tr><td colspan="100%" class="text-center opacity-70 text-white">Loading sales items...</td></tr>`;
        fetchmakesaless()
    }else{
        ...
        document.getElementById('entrybody').innerHTML = `<tr><td colspan="100%" class="text-center opacity-70 text-white">Awaiting for source branch and department to be set...</td></tr>`;
    }
}


async function fetchmakesaless() {
    thedepartment = '';
    thebranch = '';
    // if(!validateForm('makesalesform', getIdFromCls('comp'))) return
    ...
    if(request.status) {
            if(request.data.length) {
                if(!did('departmentfrom').value)return notification('Sorry! Please re-select the department', 0);
                // datasource = request.data;
                themakesalesitems = request.data;
                console.log('themakesalesitems', themakesalesitems)
                ...
                <tr id="reqrow_987" class="bg-white hover:bg-gray-100 transition-transform duration-300 transform hover:scale-105">
                    <td class="text-center p-4 " style="width: 70%;"> 
                        <label for="supplyfrom_${elid}" class="sr-only">Item</label>
                        <select name="" id="supplyfrom_${elid}" class="form-controls w-full p-3  rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                            <option value=''>-- Select item --</option>
                            ${themakesalesitems.map(data => `<option value='${data.itemid}'>${data.itemname}</option>`).join('')}
                        </select>
                    </td>
                    <td class="text-center p-4 border-b border-gray-200" style="width: 30%;">
                        <label for="qty_0" class="sr-only">Quantity</label>
                        <input onchange="makesalescal2(this)" type="number" id="qty_${elid}" name="" class="form-control bg-white w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none" placeholder="Qty" min="1">
                    </td>
                    <td class="text-center p-4 border-b border-gray-200">
                        <div class="flex justify-center items-center">
                            <button onclick="entryreqaddrow()" title="Add row entry" class="material-symbols-outlined rounded-full bg-green-500 h-10 w-10 text-white shadow-md text-sm hover:bg-green-600 transition-transform duration-300 transform hover:scale-110 flex justify-center items-center" style="font-size: 18px;">
                                add
                            </button>
                        </div>
                    </td>
                ...
                // resolvePagination(datasource, onmakesalesTableDataSignal)
...
async function fetchviewsalesday() {
    ...
    let formData = new FormData(document.getElementById('viewsalesdayform'));
    ...
    let request = await httpRequest2(`api/v1/sales/viewsalesday?${queryParams ? `${queryParams}` : ''}`, null, document.querySelector('#viewsalesdaySubmit'), 'json', 'GET');
    if(request.status) {
            if(request.data) {
                        ...
                        return `
                            <tr>
                                ...
                                <td class="flex items-center gap-3 justify-center">
                                    <button title="View Transaction" onclick="viewTransaction('${group.transaction.transactionref}')" 
                                    class="material-symbols-outlined rounded-full bg-blue-500 h-8 w-8 text-white drop-shadow-md text-lg hover:bg-blue-600">
                                    visibility
                                    </button>
                                    <button title="Print Receipt" onclick="printReceipt('${group.transaction.transactionref}')" 
                                    ...
                                    <button title="Download Receipt" onclick="downloadReceipt('${group.transaction.transactionref}')" 
...
async function fetchviewsalesmonth() {
    ...
    let formData = new FormData(document.getElementById('viewsalesmonthform'));
    formData.append('userid', the_user.id);
    let queryParams = new URLSearchParams(formData).toString();
    ...
    let request = await httpRequest2(`api/v1/sales/viewsalesmonth?${queryParams ? `${queryParams}` : ''}`, null, document.querySelector('#viewsalesmonthSubmit'), 'json', 'GET');
    if(request.status) {
            if(request.data) {
...
async function fetchviewsalesyear() {
    ...
    let formData = new FormData(document.getElementById('viewsalesyearform'));
    let queryParams = new URLSearchParams(formData).toString();

    // Fetch data from the API
    let request = await httpRequest2(`api/v1/sales/viewsalesyear?${queryParams ? `${queryParams}` : ''}`, null, document.querySelector('#viewsalesyearSubmit'), 'json', 'GET');
    if (request.status) {
        if (request.data) {
...
function viewTransaction(reference) {
    ...
    Swal.fire({
        ...
        <div class="flex gap-3 w-full">
            ...
            <button onclick="printReceipt('${reference}')" class="swal-confirm-btn flex-1 px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors">
                Print Receipt
            </button>
        </div>
        `
...
function printReceipt(reference) {
  ...
  // Usage example: 
  // downloadReceipt('1738010399196', yourSalesDataObject)
...
async function reqaddrowmakesales(){
    console.log('themakesalesitems', themakesalesitems)
    if (!themakesalesitems.length) return notification('makesales Source is incomplete. Wait for the table to indicate readiness.', 0);
    let el = document.createElement('tr');
    el.classList.add('temprow');
    let elid;

    if (themakesalesitems.length) {
        elid = genID();
        el.id = `reqrow_${elid}`;
        let x = `
            <td class="text-center opacity-70 w-[200px]"> 
                <label class="hidden">Item</label>
                <select onchange="makesalesgetitemdetails(this)" name="supplyfrom" id="supplyfrom_${elid}" class="form-controls comp w-[220px]">
                    <option value=''>-- Select item --</option>
                    ${themakesalesitems.map(data => `<option value='${data.itemid}'>${data.itemname}</option>`).join('')}
                </select>
            </td>
            ...
            <td>
                ...
                <input onchange="makesalescal2(this)" type="number" id="cost_${elid}" name="cost" class="comma form-control comp w-[95px]" readonly placeholder="Cost">
            </td>
            <td>
                <label class="hidden">Price</label>
                <input onchange="makesalescal2(this)" type="number" readonly id="price_${elid}" name="price" class="comma form-control comp w-[95px]" placeholder="Enter Price">
            </td>
            <td>
                <label class="hidden">Quantity</label>
                <input onchange="makesalescal2(this)" type="number" id="qty_${elid}" name="qty" class="comma form-control comp w-[70px]" placeholder="Qty">
            </td>
            <td>
            ...
            <td>
                <div class="flex gap-4 items-center h-full w-fit py-3">
                    <button onclick="reqaddrowmakesales()" title="Add row" class="hidden material-symbols-outlined rounded-full bg-primary-g h-8 w-8 text-white drop-shadow-md text-xs" style="font-size: 18px;">add</button>
...
function makesalescal2(element){
    dynamiccomma(false)
...
async function makesalesgetitemdetails(el){
    console.log('makesalesgetitemdetails', el, el.value, el.id)  
    ...
    let req = themakesalesitems.filter(data=>data.itemid == el.value)[0]
    ...
    if(request.status) {
            if(request.data.length) {
                // datasource = request.data;
                themakesalesitems = request.data;
                let req2 = themakesalesitems.filter(data=>data.itemid == el.value)[0]
...
async function onmakesalesTableDataSignal() {
...
async function makesalesFormEditHandler(id='') {
    dynamiccomma(false);
    if (!validateForm('makesalesform', getIdFromCls('comp'))) return;
...
const confirm = await Swal.fire({
    ...
    let request = await httpRequest2('api/v1/sales/makesales', payload, document.querySelector('#save'), 'json', 'POST');
    Swal.close(); // Close SweetAlert processing
    
    if (request.status) {
        makesalesActive();
        document.getElementById('makesales').click();
            ...
            const { isConfirmed } = await Swal.fire({
                ...
                didOpen: () => {
                    ...
                    margin-top: 10px;
                        ...
                        font-size: 14px;
                        `;
                        downloadButton.onclick = () => downloadReceipt(receiptContent);
        ...
        dynamiccomma(true);
        return notification('Failed to perform the required makesales', 0);
    }
}



async function makesalesFormDeleteHandler(id='') {
    // Show SweetAlert processing
    Swal.fire({
        title: 'Processing',
...
// function runAdmakesalesFormValidations() {
//     let form = document.getElementById('makesalesform')
//     let errorElements = form.querySelectorAll('.control-error')
//     let controls = []

//     if(controlHasValue(form, '#owner'))  controls.push([form.querySelector('#owner'), 'Select an owner'])
//     if(controlHasValue(form, '#makesalesname'))  controls.push([form.querySelector('#makesalesname'), 'makesales name is required'])
