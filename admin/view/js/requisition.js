...
async function requisitionActive() {
    ...
    // if(document.querySelector('#reqsubmit')) document.querySelector('#reqsubmit').addEventListener('click', e=>fetchrequisitions())
    if(document.querySelector('#reqsubmit')) document.querySelector('#reqsubmit').addEventListener('click', e=>requisitionFormEditHandler())
    ...
    // if(document.querySelector('#save')) document.querySelector('#save').addEventListener('click', e=>requisitionFormEditHandler())
...
async function fetchrequisitions() {
    thedepartment = '';
    thebranch = '';
    // if(!validateForm('requisitionform', getIdFromCls('comp'))) return
    ...
    if(request.status) {
            if(request.data.length) {
                ...
                therequisitionitems = request.data;
                console.log('therequisitionitems', therequisitionitems)
                ...
                // resolvePagination(datasource, onrequisitionTableDataSignal)
            }
        }
...
function reqaddrowrequisition(){
    console.log('therequisitionitems', therequisitionitems)
    if (!therequisitionitems.length) return notification('Requisition Source is incomplete. Wait for the table to indicate readiness.', 0);
    let el = document.createElement('tr');
    el.classList.add('temprow');
    let elid;

    if (therequisitionitems.length) {
        elid = genID();
        el.id = `reqrow_${elid}`;
        let x = `
            <td class="text-center opacity-70 w-[200px]"> 
                <label class="hidden">Item</label>
                <select onchange="getitemdetails(this)" name="supplyfrom" id="supplyfrom_${elid}" class="form-controls comp w-[220px]">
                    <option value=''>-- Select item --</option>
                    ${therequisitionitems.map(data => `<option value='${data.itemid}'>${data.itemname}</option>`).join('')}
                </select>
            </td>
            <td>
            ...
            <td>
                ...
                <input onchange="requisitioncal2(this)" type="number" id="cost_${elid}" name="cost" class="comma form-control comp w-[95px]" readonly placeholder="Cost">
            </td>
            <td>
                <label class="hidden">Price</label>
                <input onchange="requisitioncal2(this)" type="number" id="price_${elid}" name="price" class="comma form-control comp w-[95px]" placeholder="Enter Price">
            </td>
            <td>
                <label class="hidden">Quantity</label>
                <input onchange="requisitioncal2(this)" type="number" id="qty_${elid}" name="qty" class="comma form-control comp w-[70px]" placeholder="Qty">
            </td>
            <td>
            ...
            <td>
                <div class="flex gap-4 items-center h-full w-fit py-3">
                    <button onclick="reqaddrowrequisition()" title="Add row" class="hidden material-symbols-outlined rounded-full bg-primary-g h-8 w-8 text-white drop-shadow-md text-xs" style="font-size: 18px;">add</button>
...
async function getitemdetails(el){
    console.log('getitemdetails', el, el.value, el.id)  
    ...
    let req = therequisitionitems.filter(data=>data.itemid == el.value)[0]
    ...
    if(request.status) {
            if(request.data.length) {
                // datasource = request.data;
                therequisitionitems = request.data;
                let req2 = therequisitionitems.filter(data=>data.itemid == el.value)[0]
...
async function requisitionFormEditHandler(id='') {
    dynamiccomma(false)
    if(!validateForm('requisitionform', getIdFromCls('comp'))) return
    ...
    let request = await httpRequest2('api/v1/inventory/requisition', payload, document.querySelector('#save'), 'json', 'POST');
    dynamiccomma(true)
    Swal.close(); // Close SweetAlert processing

    // requisitionFormSubmitHandler();
    if (request.status) {
        requisitionActive();
        document.getElementById('requisition').click()
        return notification(request.message, 1);
    } else {
        return notification('Failed to perform the required requisition', 0);
    }
}


async function requisitionFormDeleteHandler(id='') {
    // Show SweetAlert processing
    Swal.fire({
        title: 'Processing',
